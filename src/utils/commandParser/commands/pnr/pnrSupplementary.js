...
export const validSSRCodes = {
  ...
  'CTCE': 'Contact details - Email address',
  'CTCM': 'Contact details - Mobile/telephone number'
};
...
export async function handleAddSSR(cmd) {
  try {
    ...
    if (ctcMatch) {
      const [, ssrCode, airlineCode, encodedValue, paxNumStr] = ctcMatch;
      const code = ssrCode.toUpperCase();
      const airline = airlineCode.toUpperCase();
      const paxNum = parseInt(paxNumStr, 10);

      if (paxNum <= 0 || paxNum > currentPNR.passengers.length)
        return `Pasajero P${paxNum} no existe. El PNR tiene ${currentPNR.passengers.length} pasajero(s).`;

      // Decode Amadeus email encoding just for the confirmation message
      let decodedDisplay = encodedValue;
      if (code === 'CTCE') {
        decodedDisplay = encodedValue
          .replace(/\/\//g, '@')
          .replace(/\.\./g, '_')
          .replace(/\.\//g, '-');
      }
      ...
      const ssrElement = {
        ...
        airlineCode: airline,
        status: 'HK1',
        message: encodedValue, // stored as-is in Amadeus encoding
        passengerNumber: paxNum,
        addedAt: new Date(),
      };

      if (!currentPNR.ssrElements) currentPNR.ssrElements = [];

      // Replace previous CTCE or CTCM for the same passenger (one per pax)
      currentPNR.ssrElements = currentPNR.ssrElements.filter(
        s => !(s.code === code && s.passengerNumber === paxNum)
      );
      currentPNR.ssrElements.push(ssrElement);
      setCurrentPNR(currentPNR);
      ...
      if (currentPNR.id) {
        try {
          await updateDoc(doc(db, 'pnrs', currentPNR.id), {
            ...
            [`history.${Date.now()}`]: {
              command: cmd,
              result: `${code} agregado P${paxNum}: ${decodedDisplay}`,
              timestamp: new Date().toISOString(),
            },
          });
        } catch (e) { console.error('Error guardando CTC SSR:', e); }
      }
    ...
    if (!validSSRCodes[ssrCode])
      return `Error: Código SSR '${ssrCode}' no válido. Use HE SSRCODES para ver la lista.`;

    if (!passengerNumber)
      return 'Error: Debe especificar un número de pasajero. Ejemplo: SRVGML/P2';
