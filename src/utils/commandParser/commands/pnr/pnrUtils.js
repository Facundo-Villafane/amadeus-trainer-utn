...
export function formatPNRResponse(pnr) {
  ...
  if (pnr.ssrElements && pnr.ssrElements.length > 0) {
    pnr.ssrElements.forEach((ssrElement) => {
      ...
      // Si es un RQST, mostrar el mensaje completo
      if (ssrElement.code === 'RQST' && ssrElement.message) {
        ssrLine += ` ${ssrElement.message}`;
      }

      // Si es CTCE o CTCM, mostrar el valor de contacto
      if ((ssrElement.code === 'CTCE' || ssrElement.code === 'CTCM') && ssrElement.message) {
        ssrLine += ` ${ssrElement.message}`;
      }
...
export function formatERResponse(pnr) {
  ...
  if (pnr.ssrElements && pnr.ssrElements.length > 0) {
    pnr.ssrElements.forEach((ssrElement) => {
      ...
      // Si es un RQST o CTC, mostrar el mensaje completo
      if ((ssrElement.code === 'RQST' || ssrElement.code === 'CTCE' || ssrElement.code === 'CTCM') && ssrElement.message) {
        ssrLine += ` ${ssrElement.message}`;
      }

      // Añadir referencia al segmento, excepto para FOID y CTC que no están asociados a segmentos
      if (ssrElement.code !== 'FOID' && ssrElement.code !== 'CTCE' && ssrElement.code !== 'CTCM' && ssrElement.segmentNumber) {
        ssrLine += ` /S${ssrElement.segmentNumber}`;
      }
  ...
  // Comentarios generales (RM) — visibles para todos
  if (pnr.remarks && pnr.remarks.length > 0) {
    pnr.remarks.forEach((remark) => {
      response += `${elementNumber} RM ${remark.text}\n`;
      elementNumber++;
  ...
  // Comentarios confidenciales (RC) — solo de la agencia
  if (pnr.confidentialRemarks && pnr.confidentialRemarks.length > 0) {
    pnr.confidentialRemarks.forEach((remark) => {
      response += `${elementNumber} RC ${remark.text}\n`;
      elementNumber++;
  ...
  // Comentarios de itinerario (RIR) — aparecen en el ticket del pax
  if (pnr.itineraryRemarks && pnr.itineraryRemarks.length > 0) {
    pnr.itineraryRemarks.forEach((remark) => {
      response += `${elementNumber} RIR ${remark.text}\n`;
      elementNumber++;
