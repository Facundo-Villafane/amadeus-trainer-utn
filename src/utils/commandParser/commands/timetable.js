...
export async function handleTimetableCommand(cmd) {
  try {
    ...
    querySnapshot.forEach((doc) => {
      ...
      const daysStr = String(daysOfOperation).padEnd(8, ' ');
      ...
      const stopsStr = '0'.padEnd(3, ' ');
      const datesStr = `${validFrom} ${validTo}`.padEnd(16, ' ');
      const equipStr = `${aircraftCode}`.padEnd(5, ' ');
      const durStr = duration.padStart(5, ' ');

      const flightLine = ` ${idxStr}   ${carrierStr}${daysStr}${routingStr}${depTimeStr}${arrTimeStr}${stopsStr}${datesStr}${equipStr}${durStr}\n`;
